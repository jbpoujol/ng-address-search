...
export class DataGouvHttpService {
  protected ROOT_URL: string = 'https://api-adresse.data.gouv.fr/';

  /**
   * Allow to change the uri if we host our own addressDataGouv service
   */
  set uri(uri: string) {
    this.ROOT_URL = uri;
  }
  ...
  protected get(loadOptions: any): Observable<AddressAPIResult[]> {
    ...
    if (loadOptions.autocomplete) {
      options.params = options.params.set(
        'autocomplete',
        loadOptions.autocomplete.toString()
      );
    }
    ...
    return this.api.get<any>(this.urlSearch, options).pipe(
